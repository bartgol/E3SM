...
struct CaarFunctorImpl {
  ...
  // using ScalarColM = decltype(Homme::subview(std::declval<ScalarM(),0,0,0));
  ...
  {
    char* team_size_str  = std::getenv("CAAR_TEAM_SIZE");
    char* vec_length_str = std::getenv("CAAR_VECTOR_LENGTH");
    if (team_size_str!=nullptr && vec_length_str!=nullptr) {
      ...
      printf ("CAAR: honoring env-var requested team_size/vector_length: %d, %d\n",team_size,vec_length);
      printf ("      Note: default values were team_size/vector_length: %d, %d\n",m_policy_pre.team_size(),get_vector_length(m_policy_pre));
      m_policy_pre = decltype(m_policy_pre)(elements.num_elems(),team_size,vec_length);
      m_policy_dp3d_lim = decltype(m_policy_dp3d_lim)(elements.num_elems(),team_size,vec_length);
    }
  ...
  KOKKOS_INLINE_FUNCTION
  void compute_pi_omega_and_phi(KernelVariables &kv) const {
    kv.team_barrier();
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team,NP*NP),
                         [&](const int idx) {
      const int igp = idx / NP; 
