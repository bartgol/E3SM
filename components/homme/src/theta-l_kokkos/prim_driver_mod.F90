...
contains
  ...
  subroutine prim_create_c_data_structures (tl, hvcoord, mp)
    ...
    use hybvcoord_mod, only : hvcoord_t
    use time_mod,      only : timelevel_t
    use theta_f2c_mod, only : init_simulation_params_c, init_elements_c, &
                              init_hvcoord_c, init_time_level_c
    !
    ! Inputs
    !
    ...
    real (kind=real_kind), target, dimension(np,np) :: elem_mp
  ...
  subroutine prim_init_elements_views(elem)
    ...
    use theta_f2c_mod, only : init_reference_element_c, init_reference_states_c, &
                              init_elements_states_c, init_elements_2d_c,        &
                              init_diagnostics_c 
    !
    ! Inputs
    !
  ...
  subroutine prim_run_subcycle(elem, hybrid, nets, nete, dt, single_column, tl, hvcoord,nsubstep)
    ...
    use prim_state_mod, only : prim_printstate
    use theta_f2c_mod,  only : push_forcing_to_c, prim_run_subcycle_c, cxx_push_results_to_f90
