...
! =========== PRIMITIVE-EQUATION DATA-STRUCTURES =====================
  ...
  type, public :: elem_state_t
    ...
    real (kind=real_kind) :: v         (:,:,:,:,:)        ! horizontal velocity 
    real (kind=real_kind) :: w_i       (:,:,:,:)          ! vertical velocity at interfaces
    real (kind=real_kind) :: vtheta_dp (:,:,:,:)          ! virtual potential temperature (mass)
    real (kind=real_kind) :: phinh_i   (:,:,:,:)          ! geopotential used by NH model at interfaces
    real (kind=real_kind) :: dp3d      (:,:,:,:)          ! delta p on levels                  
    real (kind=real_kind) :: ps_v      (:,:,:)            ! surface pressure                   
    real (kind=real_kind) :: phis      (:,:)              ! surface geopotential (prescribed)  
    real (kind=real_kind) :: Q         (:,:,:,:)          ! Tracer concentration               
    real (kind=real_kind) :: Qdp       (:,:,:,:,:)        ! Tracer mass                        
